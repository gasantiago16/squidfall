...
- **GitLab (for the Army instance):** the same pipeline is translated in **`.gitlab-ci.yml`** for a **shell-executor** GitLab Runner (build → test/lint → Trivy → deploy on `v*` tags, with rollback). CD pushes to GitLab's built-in **Container Registry** (`$CI_REGISTRY_IMAGE`). **Air-gap-ready:** every external pull is overridable via CI/CD variables — `REGISTRY_PREFIX` (base images), `PIP_INDEX_URL`/`PIP_TRUSTED_HOST` (PyPI), `NPM_REGISTRY` (npm), `APK_MIRROR` (alpine), `RUFF_IMAGE`/`TRIVY_IMAGE` (tool images). Inject `GEOCODING_API_KEY` masked; set the runner `tag`. The `.github/workflows/` files are ignored on GitLab.
